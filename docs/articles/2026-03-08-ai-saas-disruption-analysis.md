...
幾個數據可以說明這股浪潮的規模。Claude Code 於 2025 年 5 月發布後，僅八個月就成為最受歡迎的 AI 編程工具，在開發者滿意度調查中以 46% 的喜愛度遙遙領先 Cursor（19%）和 GitHub Copilot（9%）。Cursor 的年化營收在不到 24 個月內突破 10 億美元，到 2026 年 3 月已超過 20 億美元，打破了 SaaS 產業所有成長紀錄。約 4% 的 GitHub commits 現在由 Claude Code 撰寫。95% 的受訪開發者每週至少使用一次 AI 工具，75% 的人有超過一半的工作由 AI 輔助完成。

這些不只是「開發者用了新工具」的數據，而是整個軟體開發方式正在發生典範轉移的訊號。
...
SaaStr 的 Jason Lemkin 精準總結了實際情況：企業不是在用 vibe coding 建造自己的 Salesforce，而是在取代那些「每月 49 美元、提供 80% 你需要的功能和 40% 你不需要的功能」的小型 SaaS 工具。
...
開源專案的價值正在發生質變。正如身份驗證服務 ZITADEL 所述，他們真正販賣的不是程式碼，而是風險轉移——承擔正常運作時間、安全回應、合規文件和長期維護的責任。SLA、SOC 2 報告、CVE 揭露、威脅情報，這些不是功能，而是保險。
...
第三種最接近「配方與材料」的完整形態：Claude Code 引入了 Agent Skills（持久性、可重用的指令）和 Plugins（將 MCP 伺服器、資源和 skills 打包為可安裝套件）。這代表了一種新的分發機制——不只分享工具，還分享 AI 代理人正確使用工具所需的指令和脈絡。
